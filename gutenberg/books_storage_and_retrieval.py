...
def main():
    ...
    # Arguements
    # first arguement is flag shorthand, followed by explanatory longhand, then specifications. "action" stores value as true to run when flag is added
    ...
    load_dotenv(override=True)  # from .env
    ...
    chat_llm = ChatOpenAI(
        ...
        temperature=0, # set responses to be high predictability and focused
        openai_api_key=OPENAI_API_KEY
    )

    # Vector store that will hold embeddings
    vector_store = SupabaseVectorStore(
       client=supabase_client,
       embedding=embeddings,
       table_name="books",
       query_name="match_books" # SQL function added directly to Supabase
    )

    # Initialize Gutenberg cache
    cache = GutenbergCache.get_cache()

    # Check for load_books flag
    if args.load_books:
